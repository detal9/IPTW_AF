...
# L2 = rbinom(n, 1, 0.5);
# A = rbinom(n, 1, plogis(2 - 0.1*L1 + L2));
# TtE04 = pmin(1/lambdaE**(1/aE)*(-log(1 - runif(n))/exp(1.4 + 0.1*L1))**(1/aE), 4);
# TtE4p = 1/lambdaE**(1/aE)*(-log(1 - runif(n))/exp(3 + A + 0.1*L1 + L2))**(1/aE);
# TtE4p_0 = 1/lambdaE**(1/aE)*(-log(1 - runif(n))/exp(3 + 0 + 0.1*L1 + L2))**(1/aE);
# TtE = TtE04 + 1*(TtE04 == 4)*TtE4p;
# TtE_0 = TtE04 + 1*(TtE04 == 4)*TtE4p_0;
...
for(i in 1:nrep){
  ...
  TtE4p = 1/lambdaE**(1/aE)*(-log(1 - runif(n))/exp(3 + A + 0.1*L1 + L2))**(1/aE);
  TtE = TtE04 + 1*(TtE04 == 4)*TtE4p;
  TtC = rlnorm(n, meanlog = log(5), sdlog = log(1.2));
...
file.name = paste("results_scenario4_",gsub("-","",Sys.Date()),".Rdata", sep="");
